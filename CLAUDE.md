...
`*.server.ts` files carry `import "server-only"`; non-suffixed `src/lib` files (`visibility.ts`, `loan-interest.ts`, `loan-party.ts`) are pure logic shared by both sides.
...
- Server Components for reads (no `useEffect` data fetching); Client Components (`"use client"`) only for interactivity or `onSnapshot`.
